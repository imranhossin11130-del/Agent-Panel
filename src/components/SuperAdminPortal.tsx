...
interface SuperAdminPortalProps {
  ...
  activeCommissionWithdrawId?: string | null;
  onClearCommissionWithdrawId?: () => void;
...
export const SuperAdminPortal: React.FC<SuperAdminPortalProps> = ({
  ...
  const [selectedCommissionRequest, setSelectedCommissionRequest] = useState<any | null>(null);
  
  // Tab control
  const [adminTab, setAdminTab] = useState<'deposits' | 'commissions'>('deposits');
  ...
  const [rejectCommissionReq, setRejectCommissionReq] = useState<any | null>(null);
  ...
  const fetchAllRequests = async () => {
    ...
    try {
      // 1. Fetch pending deposit requests
      const qDep = query(
        collection(db, 'deposit_requests'),
        where('status', '==', 'pending')
      );
      const querySnap = await getDocs(qDep);
      const list: any[] = [];
      ...
      // 2. Fetch pending commission withdrawals
      const qComm = query(
        collection(db, 'commission_withdrawals'),
        where('status', '==', 'pending')
      );
      ...
      setCommissionRequests(commList);

      // Link redirection checking for deposit validation
      if (activeRequestId) {
        ...
        if (found) {
          ...
          if (singleSnap.exists()) {
            ...
            onShowToast(isBn ? 'ডিপোজিট অনুরোধটি পাওয়া যায়নি!' : 'Requested deposit not found!', 'info');
          }
        }
      }

      // Link redirection checking for commission withdrawal validation
      if (activeCommissionWithdrawId) {
        setAdminTab('commissions');
        const found = commList.find(r => r.id === activeCommissionWithdrawId);
        if (found) {
          setSelectedCommissionRequest(found);
        } else {
          const singleRef = doc(db, 'commission_withdrawals', activeCommissionWithdrawId);
          const singleSnap = await getDoc(singleRef);
          if (singleSnap.exists()) {
            setSelectedCommissionRequest({ id: singleSnap.id, ...singleSnap.data() });
          } else {
            onShowToast(isBn ? 'উত্তোলন অনুরোধটি পাওয়া যায়নি!' : 'Requested commission withdraw not found!', 'info');
          }
        }
      }
  ...
  useEffect(() => {
    fetchAllRequests();
  }, [activeRequestId, activeCommissionWithdrawId]);

  // Approve action flow for Deposit Requests
  ...
  // Reject action flow for Deposit Requests
  const handleReject = async (req: any) => {
    ...
    try {
      ...
      console.error(err);
      setErrorMessage(isBn ? 'প্রত্যাখ্যান প্রক্রিয়া সম্পন্ন করতে समस्या হয়েছে।' : 'Failed to reject request.');
    } finally {
      setLoading(false);
      setRejectConfirmationRequest(null);
    }
  ...
  // Approve action flow for Commission Withdrawal Requests
  const handleApproveCommission = async (req: any) => {
    setLoading(true);
    setErrorMessage(null);
    try {
      // 1. Load Agent Document to check and decrement commission balance
      const agentRef = doc(db, 'agents', req.agentUid);
      const agentSnap = await getDoc(agentRef);
      if (!agentSnap.exists()) {
      ...
      const agentData = agentSnap.data();
      const currentComm = Number(agentData.commissionBalance || 0);
      const requestedComm = Number(req.amount || 0);

      if (currentComm < requestedComm) {
        setErrorMessage(isBn 
          ? `দুঃখিত! এজেন্টের কমিশন ব্যালেন্স পর্যাপ্ত নেই (বর্তমান ব্যালেন্স: ৳${currentComm})।` 
          : `Insufficient balance! Agent only has ৳${currentComm} commission.`);
        setLoading(false);
        return;
      }

      // 2. Update the withdrawal request document status
      const reqRef = doc(db, 'commission_withdrawals', req.id);
      await updateDoc(reqRef, {
        status: 'success',
        updatedAt: new Date().toISOString()
      });

      // Update the Agent Profile - Deduct commission balance
      await updateDoc(agentRef, {
        commissionBalance: currentComm - requestedComm
      });

      // 3. Inject a successful transaction into agent logs
      const trxsRef = collection(db, 'agents', req.agentUid, 'transactions');
      const generatedId = req.id || 'CW' + Math.floor(100000 + Math.random() * 900000);
      await setDoc(doc(db, 'agents', req.agentUid, 'transactions', generatedId), {
        id: generatedId,
        type: 'commission_withdraw',
        phoneOrAccount: `${req.paymentMethod} (${req.phoneNumber})`,
        amount: requestedComm,
        commission: 0,
        timestamp: new Date().toISOString(),
        status: 'SUCCESS',
        operatorOrBiller: isBn ? 'কমিশন উত্তোলন অনুমোদিত' : 'Commission Withdrawal Approved'
      });

      onShowToast(isBn ? 'কমিশন উত্তোলন অনুরোধ সফলভাবে অনুমোদিত হয়েছে!' : 'Commission withdrawal approved successfully!', 'success');
      setSuccessAnimation(true);
      setTimeout(() => {
        setSuccessAnimation(false);
        setSelectedCommissionRequest(null);
        if (onClearCommissionWithdrawId) {
          onClearCommissionWithdrawId();
        }
        fetchAllRequests();
      }, 2500);
      ...
      console.error(err);
      setErrorMessage(isBn ? 'অনুমোদন প্রক্রিয়া সম্পূর্ণ করতে ত্রুটি হয়েছে।' : 'Failed to approve. Try again.');
    } finally {
      setLoading(false);
    }
  };

  // Reject action flow for Commission Withdrawal Requests
  const handleRejectCommission = async (req: any) => {
    setLoading(true);
    setErrorMessage(null);
    try {
      const reqRef = doc(db, 'commission_withdrawals', req.id);
      ...
      onShowToast(isBn ? 'কমিশন উত্তোলনের অনুরোধটি বাতিল করা হয়েছে!' : 'Commission withdrawal request rejected!', 'success');
      setSelectedCommissionRequest(null);
      setRejectCommissionReq(null);
      if (onClearCommissionWithdrawId) {
        onClearCommissionWithdrawId();
      }
      fetchAllRequests();
      ...
      console.error(err);
      setErrorMessage(isBn ? 'অনুরোধ বাতিল করতে ত্রুটি হয়েছে।' : 'Failed to reject request.');
    } finally {
      setLoading(false);
      setRejectCommissionReq(null);
    }
  ...
  return (
    <div className="flex-1 flex flex-col justify-between overflow-hidden relative select-none font-sans bg-transparent">
      ...
      <div className="p-4 bg-slate-900 border-b border-white/5 flex items-center justify-between shrink-0">
        <div className="flex items-center space-x-2">
          ...
          <div>
            ...
            <h2 className="text-xs font-black text-white/90 uppercase tracking-widest mt-0.5">
              {isBn ? 'অ্যাডমিন কন্ট্রোল পোর্টাল' : 'VAL-PORTAL GATEWAY'}
            </h2>
          </div>
        </div>
        ...
        <button 
          onClick={onLogout}
          className="p-2.5 bg-white/5 hover:bg-white/10 rounded-xl border border-white/10 text-white/60 hover:text-white transition-all active:scale-95 flex items-center gap-1 cursor-pointer text-[10px] font-black tracking-wider uppercase text-rose-350"
        >
          <LogOut size={13} />
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4">
        ...
        {successAnimation ? (
          <div className="flex flex-col items-center justify-center py-20 text-center space-y-4 animate-fade-in select-none">
            ...
            <div>
              ...
              <p className="text-[10px] text-white/45 max-w-[250px] mx-auto mt-1 leading-relaxed">
                {isBn 
                  ? 'উক্ত আবেদনটি সফল করা হয়েছে এবং ব্যালেন্স সমন্বয় করা হয়েছে।' 
                  : 'Balances loaded and successfully synced with secure databases.'}
              </p>
            </div>
          </div>
        ) : (selectedRequest || selectedCommissionRequest) ? (
          /* Focused detailed view depending which is selected */
          selectedRequest ? (
            /* Deposit detailed view */
            <div className="glass-card rounded-2xl p-4.5 border border-white/15 space-y-4 select-none animate-fade-in bg-slate-900/50">
              {/* Context meta */}
            ...
            <div className="glass-card rounded-2xl p-4.5 border border-white/15 space-y-4 select-none animate-fade-in bg-slate-900/50">
              ...
              <div className="flex justify-between items-center pb-2.5 border-b border-white/5 shrink-0">
                <button
                  onClick={() => {
                    setSelectedCommissionRequest(null);
                    if (onClearCommissionWithdrawId) onClearCommissionWithdrawId();
                  }}
                  className="py-1 px-2.5 bg-white/5 hover:bg-white/10 text-white/60 rounded-lg text-[9px] uppercase font-black tracking-widest cursor-pointer"
                >
                  {isBn ? 'তালিকায় ফিরুন' : 'Back to list'}
                ...
                <span className="text-[9px] font-mono text-rose-350 tracking-wider">REQ: #{selectedCommissionRequest.id.substring(0, 10).toUpperCase()}</span>
              </div>
              ...
              <div className="space-y-2.5 text-xs">
                <div className="flex justify-between items-center">
                  <span className="text-white/45">{isBn ? 'এজেন্ট পার্টনার' : 'Agent Partner'}</span>
                  <span className="font-extrabold text-white">{selectedCommissionRequest.agentName || "Agent Partner"}</span>
                </div>
                <div className="flex justify-between items-center">
                  <span className="text-white/45">{isBn ? 'এজেণ্টের মোবাইল' : 'Agent Phone'}</span>
                  <span className="font-bold text-white tracking-widest font-mono">{selectedCommissionRequest.agentPhone}</span>
                </div>
                <div className="flex justify-between items-center pt-2 border-t border-white/5">
                  <span className="text-white/45">{isBn ? 'পেমেন্ট মেথড' : 'Payment Method'}</span>
                  <span className="font-black text-rose-350 uppercase px-2 py-0.5 bg-rose-500/10 rounded-md border border-rose-500/15 text-[10px]">
                    {selectedCommissionRequest.paymentMethod}
                  </span>
                </div>
                <div className="flex justify-between items-center">
                  <span className="text-white/45">{isBn ? 'উত্তোলন নাম্বার' : 'Recipient Phone'}</span>
                  <span className="font-bold text-white tracking-widest font-mono">{selectedCommissionRequest.phoneNumber}</span>
                </div>

                <div className="flex justify-between items-center pt-2 border-t border-white/5 text-[13px] font-black">
                  <span className="text-white/50">{isBn ? 'অনুরোধকৃত উত্তোলনের পরিমাণ' : 'Withdraw Amount'}</span>
                  <span className="text-rose-350 tracking-wider font-extrabold">৳ {Number(selectedCommissionRequest.amount).toLocaleString()}</span>
                </div>
              </div>
              ...
              {selectedCommissionRequest.status !== 'pending' ? (
                <div className="pt-3 border-t border-white/5 text-center">
                  <div className={`p-3 rounded-xl border font-black text-xs flex items-center justify-center gap-2 ${
                    selectedCommissionRequest.status === 'success' 
                      ? 'bg-emerald-500/10 text-emerald-300 border-emerald-500/20' 
                      : 'bg-red-500/10 text-red-300 border-red-500/20'
                  }`}>
                    <CheckCircle size={15} className={selectedCommissionRequest.status === 'success' ? 'text-emerald-400' : 'text-red-400'} />
                    <span>
                      {selectedCommissionRequest.status === 'success' 
                        ? (isBn ? 'এই আবেদনটি ইতিমধ্যে অনুমোদিত হয়েছে!' : 'This request has already been approved!')
                        : (isBn ? 'এই আবেদনটি ইতিমধ্যে প্রত্যাখ্যান করা হয়েছে!' : 'This request has already been rejected!')
                ...
                <div className="pt-3 border-t border-white/5 flex gap-3 select-none">
                  <button
                    type="button"
                    onClick={() => setRejectCommissionReq(selectedCommissionRequest)}
                    disabled={loading}
                    className="flex-1 py-3 bg-red-500/15 border border-red-500/20 text-rose-300 rounded-xl font-black text-xs uppercase cursor-pointer transition-all hover:bg-red-500/25 active:scale-95 disabled:opacity-50"
                  >
                    {isBn ? 'প্রত্যাখ্যান করুন' : 'Reject'}
                  ...
                  <button
                    type="button"
                    onClick={() => handleApproveCommission(selectedCommissionRequest)}
                    disabled={loading}
                    className="flex-1.5 py-3 bg-gradient-to-r from-rose-500 to-pink-650 text-white rounded-xl font-black text-xs uppercase cursor-pointer transition-all hover:opacity-90 active:scale-95 disabled:opacity-50 flex items-center justify-center gap-1 active-glow"
                  >
                    <Check size={14} />
          ...
          /* General dashboard request list view with tab switcher */
          <div className="space-y-4">
            
            {/* Tab switch buttons */}
            <div className="flex bg-slate-950 p-1.5 rounded-2xl border border-white/5 mb-2.5 shrink-0">
              <button
                type="button"
                onClick={() => setAdminTab('deposits')}
                className={`flex-1 py-2.5 text-xs font-black rounded-xl transition-all cursor-pointer flex items-center justify-center space-x-1.5 ${
                  adminTab === 'deposits'
                    ? 'bg-rose-500/15 text-rose-300 font-extrabold shadow-sm'
                    : 'text-white/40 hover:text-white/60'
                }`}
              >
                <Coins size={14} />
                <span>{isBn ? 'ডিপোজিট আবেদন' : 'Deposits'}</span>
                {requests.length > 0 && (
              ...
              >
                <Percent size={14} />
                <span>{isBn ? 'কমিশন উত্তোলন' : 'Commissions'}</span>
            ...
            {loading ? (
              <div className="flex flex-col items-center justify-center py-12 space-y-2">
                ...
                <span className="text-[9px] uppercase tracking-wider font-extrabold text-white/40">{isBn ? 'লোড হচ্ছে...' : 'Loading Data...'}</span>
              </div>
            ) : adminTab === 'deposits' ? (
              /* Deposits List */
              requests.length === 0 ? (
                <div className="text-center py-12 bg-white/5 border border-white/5 rounded-2xl select-none text-white/40 space-y-1.5">
                  ...
                  <p className="text-xs font-bold text-white">{isBn ? 'কোন অপেক্ষমাণ ডিপোজিট অনুরোধ নেই!' : 'No pending deposit requests!'}</p>
                ...
                <div className="grid grid-cols-1 gap-2.5 select-none">
                  {requests.map((req) => (
                    <button
                      ...
                      className="glass-card p-4 border border-white/10 hover:border-white/15 hover:bg-white/10 rounded-2xl text-left transition-all active:scale-98 cursor-pointer flex items-center justify-between bg-[#111928]/40"
                    >
                      <div className="space-y-1 pr-6 truncate flex-1 font-sans">
                        <div className="flex items-center space-x-1.5 text-[10px] font-black uppercase text-rose-350">
                          <span>{req.agentName || "Agent Partner"}</span>
                          <span>•</span>
                          <span className="font-mono tracking-wider text-white/50">{req.agentPhone}</span>
                        </div>
                        <p className="text-normal font-extrabold text-white">৳ {Number(req.amount).toLocaleString()}</p>
              ...
              /* Commissions Withdrawals List */
              commissionRequests.length === 0 ? (
                <div className="text-center py-12 bg-white/5 border border-white/5 rounded-2xl select-none text-white/40 space-y-1.5">
                  <CheckCircle size={22} className="mx-auto text-emerald-350" />
                  <p className="text-xs font-bold text-white">{isBn ? 'কোন অপেক্ষমাণ কমিশন উত্তোলনের অনুরোধ নেই!' : 'No pending commission withdrawals!'}</p>
                  <p className="text-[9px] uppercase tracking-wider font-black font-mono">{isBn ? 'সকল আবেদন নিষ্পত্তি হয়েছে' : 'No applications left'}</p>
                </div>
              ) : (
                <div className="grid grid-cols-1 gap-2.5 select-none">
                  {commissionRequests.map((req) => (
                    <button
                      key={req.id}
                      onClick={() => setSelectedCommissionRequest(req)}
                      className="glass-card p-4 border border-white/10 hover:border-white/15 hover:bg-white/10 rounded-2xl text-left transition-all active:scale-98 cursor-pointer flex items-center justify-between bg-[#111928]/45 animate-fade-in"
                    >
                      <div className="space-y-1 pr-6 truncate flex-1 font-sans">
                        <div className="flex items-center space-x-1.5 text-[10px] font-black uppercase text-rose-350">
                          <span>{req.agentName || "Agent Partner"}</span>
                          <span>•</span>
                          <span className="font-mono tracking-wider text-[#ea580c] bg-orange-500/10 px-1.5 py-0.5 rounded border border-orange-500/10">{req.paymentMethod}</span>
                        </div>
                        <p className="text-normal font-black text-white">৳ {Number(req.amount).toLocaleString()}</p>
                        <span className="text-[9px] text-white/40 block leading-tight font-mono">
                          {isBn ? 'প্রাপক মোবাইল: ' : 'Mobile: '} {req.phoneNumber}
                        </span>
                      </div>
      ...
      {/* Full screened popup modal for screenshot previewing */}
      {viewScreenshotUrl && (
      ...
      {/* Clean Custom Reject Confirmation Overlay for Deposits */}
      <AnimatePresence>
            ...
            >
              <div className="flex items-center space-x-3">
                ...
                <div>
                  <h4 className="text-sm font-black text-white uppercase tracking-wider">
                    {isBn ? 'ডিপোজিট বাতিল নিশ্চিত করুন' : 'Confirm Rejection'}
                  </h4>
              ...
              <div className="flex gap-3 pt-2">
                <button
                  ...
                  className="flex-1 py-3 bg-white/5 border border-white/10 text-white hover:bg-white/10 rounded-xl font-black text-xs uppercase cursor-pointer transition-all active:scale-95"
                >
                  {isBn ? 'ফিরে যান' : 'Cancel'}
                </button>
                <button
                  onClick={() => handleReject(rejectConfirmationRequest)}
                  className="flex-1 py-3 bg-red-650 hover:bg-red-600 text-white rounded-xl font-black text-xs uppercase cursor-pointer transition-all active:scale-95 shadow-lg shadow-red-500/20"
                >
      ...
      {/* Clean Custom Reject Confirmation Overlay for Commission Withdrawals */}
      <AnimatePresence>
        {rejectCommissionReq && (
          <motion.div
            initial={{ opacity: 0 }}
          ...
          >
            ...
            >
              <div className="flex items-center space-x-3">
                ...
                <div>
                  <h4 className="text-sm font-black text-white uppercase tracking-wider">
                    {isBn ? 'অনুরোধ বাতিল নিশ্চিত করুন' : 'Confirm Rejection'}
                  </h4>
                  <span className="text-[10px] text-white/40 font-bold block mt-0.5">
                    {isBn ? 'কমিশন উত্তোলন বাতিল নিষ্পত্তি' : 'Commission Withdraw Rejection'}
                  </span>
                </div>
              </div>

              <p className="text-xs text-white/70 font-semibold leading-relaxed font-sans">
                {isBn 
                  ? `আপনি কি নিশ্চিতভাবে ৳${Number(rejectCommissionReq.amount).toLocaleString()} কমিশন উত্তোলনের আবেদনটি প্রত্যাখ্যান করতে চান?` 
                  : `Are you sure you want to reject this commission withdrawal of ৳${Number(rejectCommissionReq.amount).toLocaleString()}?`}
              </p>

              <div className="flex gap-3 pt-2 font-sans">
                <button
                  onClick={() => setRejectCommissionReq(null)}
                  className="flex-1 py-3 bg-white/5 border border-white/10 text-white hover:bg-white/10 rounded-xl font-black text-xs uppercase cursor-pointer transition-all active:scale-95"
                >
                  {isBn ? 'ফিরে যান' : 'Cancel'}
                </button>
                <button
                  onClick={() => handleRejectCommission(rejectCommissionReq)}
                  className="flex-1 py-3 bg-red-650 hover:bg-red-600 text-white rounded-xl font-black text-xs uppercase cursor-pointer transition-all active:scale-95 shadow-lg shadow-red-500/20"
                >
                  {isBn ? 'বাতিল করুন' : 'Reject'}
                </button>
